...
import { useEmployerUserList } from "@/api/requests/dashboard/dataContributorUsers";
import { Typography } from "@mui/material";
import { useQueryClient } from "@tanstack/react-query";

interface DataContributorUsersTableProps {
  readonly searchQuery: string;
}

export default function DataContributorUsersTable({
 searchQuery,
 }:DataContributorUsersTableProps) {
  ...
  const [rows, setRows] = useState<DataContributorUsersValues[]>([]);
  ...
  const { mutateAsync:employerUserList , isPending: isLoading } =   useEmployerUserList();
  const queryClient = useQueryClient();
  const [open, setOpen] = useState(false);
  const [selectedUserId, setSelectedUserId] = useState<string | null>(null);

  const fetchData =  () => {
    // API call to fetch data contributor
    employerUserList({  page:paginationModel.page,pageSize: paginationModel.pageSize,sort: sortModel, search: searchQuery })
      .then((response) => {
